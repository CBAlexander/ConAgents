...
#import data.grid_only_drivers as GoD
...
BOT_NAME = 'grid_coherence_bot'
VERSION = "GRID-Coherence"
BRANCH = "Master"
...
parser.add_argument('-p', "--port", type=int, default=5135)
...
class CoherenceBot(Bot):
    ...
    def _get_answer(self, request_data):
        ...
        logger.info('TURN No: {}'.format(turn))
    ...
    def handle_intents(self, intent, intent_param, question, turn, topic,
        ...
        if intent == 'repeat':
            ...
            return ('STOP INTENT REQUESTED'
                    ...
                    else 'Aww. You want to stop? But I love talking with you. And I can talk more about movies or '
    ...
    def get_grid_only_driver(self):
        """ Choose a grid_only driver randomly and format it with a random opening
            Try not to advertise bots that have been used yet
        """
        request_data_str = str(DictQuery(request.get_json(force=True)))

        # Check which bots have already been used in this session
        dont_mention = []
        for topic in ["directions", "events", "resource"]:
            if "{}_bot".format(topic) in request_data_str:
                dont_mention.append(topic[0])

        # Check which drivers have already been uttered in this session
        apostrophes = set(re.findall(r"[A-Za-z0-9]+'[A-Za-z0-9]+", request_data_str))
        for a in apostrophes:
            request_data_str = request_data_str.replace(a, a.replace("'", ""))
        used_drivers = set(re.findall(r"response': {'coherence_bot': ['\"]([^'\"]+)['\"]", request_data_str))
        used_drivers_tpls = []
        for udriver in used_drivers:
            highest_sim = 0
            selection = None
            for driver in GoD.DRIVERS:
                similarity = SequenceMatcher(None, udriver, driver[1]).ratio()
                if similarity > highest_sim:
                    highest_sim = similarity
                    selection = driver
            used_drivers_tpls.append(selection)

        # Get 'grid_only' drivers and filter drivers already used
        # and drivers not to mention
        if len(used_drivers_tpls) == len(GoD.DRIVERS):  # it will be at most 9 because a longer history is not accessible
            logger.info("All drivers have already been used. Setting used_drivers_tpls to an empty list!")
            used_drivers_tpls = []
        drivers = [tpl for tpl in GoD.DRIVERS if tpl[0] not in dont_mention and tpl not in used_drivers_tpls]
        logger.info("Used \"grid only\" drivers: {0}/{2} | Available \"grid only\" drivers: {1}/{2}".format(len(used_drivers_tpls), len(drivers), len(GoD.DRIVERS)))

        # Choose a driver randomly from the remaining ones
        opening = random.choice(GoD.OPENINGS)
        try:
            choice = random.choice(drivers)
        except IndexError as e:
            logger.error("[ERROR] {} | Choosing from all drivers.".format(e))
            choice = random.choice(GoD.DRIVERS)
        driver = choice[1].format(opening[0], opening[1])
    ...
    def get_driver(self, question,
        ...
        if False: #self.config == "grid_only":
            return self.get_grid_only_driver()
